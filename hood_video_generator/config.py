...
class Settings(BaseSettings):
    ...
    total_image_count: int = Field(default=10, alias="TOTAL_IMAGE_COUNT")
    ...
    story_min_chars: int = Field(default=1250, alias="STORY_MIN_CHARS")
    story_target_chars: int = Field(default=1350, alias="STORY_TARGET_CHARS")
    story_max_chars: int = Field(default=1500, alias="STORY_MAX_CHARS")
    ...
    sound_design_enabled: bool = Field(default=True, alias="SOUND_DESIGN_ENABLED")
    sound_assets_dir: str = Field(default="assets/sounds", alias="SOUND_ASSETS_DIR")
    use_signature_hit: bool = Field(default=True, alias="USE_SIGNATURE_HIT")
    use_transition_whoosh: bool = Field(default=True, alias="USE_TRANSITION_WHOOSH")
    use_low_boom_on_twists: bool = Field(default=True, alias="USE_LOW_BOOM_ON_TWISTS")
    use_glitch_on_digital_moments: bool = Field(default=True, alias="USE_GLITCH_ON_DIGITAL_MOMENTS")
    use_riser_before_twist: bool = Field(default=False, alias="USE_RISER_BEFORE_TWIST")
    use_ambient_loop: bool = Field(default=True, alias="USE_AMBIENT_LOOP")
    use_reload_sound: bool = Field(default=False, alias="USE_RELOAD_SOUND")
    auto_convert_mp4_sounds: bool = Field(default=True, alias="AUTO_CONVERT_MP4_SOUNDS")
    converted_sound_output_dir: str = Field(default="assets/sounds/converted", alias="CONVERTED_SOUND_OUTPUT_DIR")
    signature_hit_volume: float = Field(default=0.25, alias="SIGNATURE_HIT_VOLUME")
    transition_whoosh_volume: float = Field(default=0.18, alias="TRANSITION_WHOOSH_VOLUME")
    low_boom_volume: float = Field(default=0.22, alias="LOW_BOOM_VOLUME")
    glitch_volume: float = Field(default=0.16, alias="GLITCH_VOLUME")
    riser_volume: float = Field(default=0.12, alias="RISER_VOLUME")
    ambient_volume: float = Field(default=0.06, alias="AMBIENT_VOLUME")
